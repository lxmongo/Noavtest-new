...
namespace nanovaTest.SelectMethod
{
    ...
    public sealed partial class RunTestPage : Page, IDisposable
    {
        ...
        private string calibrationFileName = "N/A";
        ChartZoomPanBehavior zoomBehavior = new ChartZoomPanBehavior();
        private bool doCalibrateflag = true;

        //RetentionTime data
        SelectTestInfo PeakInfo;
        private double NewPeakSelect = 0;
        private int NewBottomLeftOriginal = 0;
        private double NewBottomLeftSelect = 0;
        private int NewBottomRightOriginal = 0;
        private double NewBottomRightSelect = 0;
        HiLoSeries RetentionSeries = new HiLoSeries();
        private bool showBottomFlag = false;
        ...
        public void zoomPan_Loaded(object sender, global::Windows.UI.Xaml.RoutedEventArgs e)
        {
        ...
        public void zoomPan_Loaded1(object sender, global::Windows.UI.Xaml.RoutedEventArgs e)
        {
            ChartZoomPanBehavior zoomBehavior = new ChartZoomPanBehavior();
            zoomBehavior.EnablePanning = true;
            zoomBehavior.ZoomMode = Syncfusion.UI.Xaml.Charts.ZoomMode.XY;
        ...
        private async void StopCalculation_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (!ReportSavedFlag && MethodNameText != "Cleaning")
            {
                ...
                if (MethodNameText == "BTEX")
                {
                    ...
                    {
                        for (; p < peaks1.Count; p++)
                        {
                            retentionTimeThreshold = service.ChangeThresholdByTime(peaks1[p], retentionTimeThreshold);
                            if (Math.Abs(x1[peaks1[p]] - RetentionTimeList[j]) < retentionTimeThreshold)
                            {
                                ...
                                FWHMvalue = CalculateFWHM(bottoms1[2 * p], peaks1[p], bottoms1[2 * p + 1], x1, y1, y_b1);
                                break;
                            }
                        }
                        //handle null error
                        ...
                        if (VOCconcentrationList.Count == 0 || VOCconcentrationList[j] == 0)
                        {
                            currentconcen = 0 / 0d;
                        }
                    ...
                    AnalysisGrid.Visibility = Visibility;
                }
                else
                {
                    for (int j = 0; j < VOCNameList.Count; j++)
                    ...
                    if (heartcuttingNumber > 0)
                    {
                        ...
                        for (int j = 0; j < VOCNameList.Count; j++)
                        {
                            ...
                            {
                                Peak2DCount++;
                                for (int p = 0; p < peaks2.Count; p++)
                                {
                                    retentionTimeThreshold = service.ChangeThresholdByTime(peaks2[p], retentionTimeThreshold);
                                    if (Math.Abs(x2[peaks2[p]] - RetentionTime2DList[j]) < retentionTimeThreshold)
                                    {
                                        ...
                                        FWHMvalue = CalculateFWHM(bottoms2[2 * p], peaks2[p], bottoms2[2 * p + 1], x2, y2, y_b2);
                                        break;
                                    }
                                }
        ...
        private async void StartCountDown()
        {
            ...
            await Windows.Storage.FileIO.AppendTextAsync(Rawfile, "Sample Name: " + ExperienceName.Text + "\n"
                + "Operator: " + OperatorName.SelectedValue + "\n"
        ...
        private async void savePdf()
        {
            ...
            using (PdfDocument document = new PdfDocument())
            {
                ...
                double TopGrid_height = TopGrid.Height;
                ...
                var saveFile = await localFolder.CreateFileAsync(string.Format("{0}.jpg", currentDate), CreationCollisionOption.GenerateUniqueName);
                ...
                Stream imageStream2 = File.OpenRead(string.Format("{0}.jpg", currentDate));
                ...
                else if (testInfoList.Count > 5)
                {
                    ...
                    p22s = new RectangleF(Width - 20, Length * (i - 3), Width + 60, Length);
                    p23s = new RectangleF(2 * Width + (0.73f * Width), Length * (i - 3), Width, Length);
                    ...
                    graphics3.DrawString("RT (s)", font, PdfBrushes.Black, p23s);
                    ...
                    for (i = 5; i < testInfoList.Count; i++)
                    {
                        ...
                        p24s = new RectangleF(3 * Width + (0.5f * Width), Length * (i - 3) + 23, Width, Length);
                        p25s = new RectangleF(4 * Width + (0.4f * Width), Length * (i - 3) + 23, Width, Length);
                        p26s = new RectangleF(5 * Width + (0.35f * Width), Length * (i - 3) + 23, Width, Length);
                        p27s = new RectangleF(6 * Width + (0.5f * Width), Length * (i - 3) + 23, Width + 30, Length);
                ...
                //TopGrid.Height = TopGrid_height;
                // Basic_Chart.Width = gridWidth;
                //TopGrid.Width = gridWidth;
                Basic_Chart.Behaviors.Add(zoomBehavior);
                notifyPopup.Show();
            }
        }
        ...
        private void SNIPBaseline(List<double> OriginalX, List<double> OriginalY, List<double> BaseLineY, List<int> peaks, List<int> bottoms, List<double> Area, List<double> Heights, double MinY)
        {
            ...
            //LLS
            for (int i = 0 + MoveIndex; i < count; i++)
            {
            ...
            {
                for (int i = p + 1 + MoveIndex; i < count - p; i++)
                {
            ...
            if (peaks != null)
            {
                try
                {
                    for (int index = 0; index < peaks.Count; index++)
                    {
                        ...
                        for (int p2 = bottoms[2 * index]; p2 < bottoms[2 * index + 1]; p2++)
                        {
        ...
        private double CalculateFWHM(int startposition, int peakposition, int endposition, List<double> OriginalX, List<double> OriginalY, List<double> BaseLineY)
        {
            ...
            double EndX = 0;
            for (int i = peakposition; i > startposition; i--)
            {
        ...
        private async void GetConcentrationFactor(string FileName)
        {
            try
            {
                ...
                Debug.WriteLine(VOCconcentrationList.Count);
                Debug.WriteLine(VOCNameList.Count);
            }
            catch (FileNotFoundException)
            {
        ...
        private void InfoListView_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            //Add bottoms notations
            if (showBottomFlag)
            {
                Debug.WriteLine(peaks1.Count);
                Debug.WriteLine(bottoms1.Count);

                int p = 0;
                for (; p < peaks1.Count; p++)
                {
                    retentionTimeThreshold = service.ChangeThresholdByTime(peaks1[p], retentionTimeThreshold);
                    if (Math.Abs(x1[peaks1[p]] - RetentionTimeList[index]) < retentionTimeThreshold)
                    {
                        //Add retention time annotation
                        LineAnnotation annotationleft = new VerticalLineAnnotation()
                        {
                            X1 = x1[bottoms1[2 * p]],
                            CanDrag = true,
                        ...
                        //Add retention time annotation
                        LineAnnotation annotationright = new VerticalLineAnnotation()
                        {
                            X1 = x1[bottoms1[2 * p + 1]],
                            CanDrag = true,
                        ...
                        NewBottomLeftOriginal = 2 * p;
                        NewBottomRightOriginal = 2 * p + 1;
                        this.Basic_Chart.Annotations.Add(annotationleft);
                        this.Basic_Chart.Annotations.Add(annotationright);
                        break;
                    }
                }
            }
        ...
        private void SaveEachPeak_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                if (this.Basic_Chart.Annotations.Count > 2)
                {
                    if (bottoms1.Count >= NewBottomRightOriginal)
                    {
                        if (showBottomFlag)
                        {
                            //Bottom update
                            NewBottomLeftSelect = System.Convert.ToDouble(this.Basic_Chart.Annotations[1].X1);
                            Debug.WriteLine("NewBottomLeftSelect" + NewBottomLeftSelect);
                            NewBottomLeftSelect = Math.Round(NewBottomLeftSelect, 1);
                            Debug.WriteLine("NewBottomLeftSelect1" + NewBottomLeftSelect);
                            for (int i = 0; i < x1.Count; i++)
                            {
                                if (Math.Abs(x1[i] - NewBottomLeftSelect) <= 0.4)
                                {
                                    bottoms1[NewBottomLeftOriginal] = i;
                                    break;
                                }
                            }
                            NewBottomRightSelect = System.Convert.ToDouble(this.Basic_Chart.Annotations[2].X1);
                            NewBottomRightSelect = Math.Round(NewBottomRightSelect, 1);
                            for (int i = 0; i < x1.Count; i++)
                            {
                                if (Math.Abs(x1[i] - NewBottomRightSelect) <= 0.4)
                                {
                                    bottoms1[NewBottomRightOriginal] = i;
                                    break;
                                }
                            }
                        }
        ...
        private void GenerateTableAgain_Click(object sender, RoutedEventArgs e)
        {
            ...
            Heights1.Clear();
            //add new Threshold
            if (double.Parse(ThresholdInput.Text) != THRESHOLD_peak)
            {
        ...
        private void CreateIndicateLine()
        {
            ...
            //*****Add peak
            Debug.WriteLine(testInfoList.Count);
            for (int i = 0; i < testInfoList.Count; i++)
            {
                Debug.WriteLine(testInfoList[i].Time);
                TextAnnotation annotation1 = new TextAnnotation()
                {
                    Text = (i + 1).ToString(),//"Annotation",
                    X1 = testInfoList[i].Time,  //X1
                    Y1 = 10, //Y1
                };
                this.Basic_Chart.Annotations.Add(annotation1);
        ...
        private async void doCalibrate()
        {
            try
            {
                ...
                //Do Calibrate or not
                if (time - maxvalue < 1000000) doCalibrateflag = false;
        ...
        {
            Debug.WriteLine("This is a import file click");
            x1.Clear();
            y1.Clear();
            y_b1.Clear();
            peaks1.Clear();
            ...
            try
            {
                ...
                reader.ReadBytes(fileContent);
                string text = GetEncoding(new byte[4] { fileContent[0], fileContent[1], fileContent[2], fileContent[3] }).GetString(fileContent);
                String[] parsedParameters = text.Split('\n');
                if (parsedParameters.Length <= 7)
                {
                    MessageDialog popup = new MessageDialog("Import file is empty!");
                    await popup.ShowAsync();
                }
                else
                {
                    //Get sampling time
                    string sampleTmp = parsedParameters[3];
                    string[] sampleTmp2 = sampleTmp.Split(':');
                    sampleTmp = sampleTmp2[1];
                    Debug.WriteLine("sampleTmp" + sampleTmp);
                    Sampletimeuwp = double.Parse(sampleTmp);
                    //Loop to get the points and baseline value
                    for (int i = 6; i < parsedParameters.Length - 1; i++)
                    {
                        string[] tmp = parsedParameters[i].Split(',');
                        for (int j = 0; j < tmp.Length; j++)
                        {
                            switch (j)
                            {
                                case 0:
                                    x1.Add(double.Parse(tmp[j]));
                                    break;
                                case 1:
                                    y1.Add(double.Parse(tmp[j]));
                                    break;
                                case 2:
                                    y_b1.Add(double.Parse(tmp[j]));
                                    break;
                                default:
                                    break;
                            }
                    ...
                    for (int i = 0; i < x1.Count && i < y_b1.Count; i++)
                    {
                        standardSource.Add(new Data(x1[i], y_b1[i]));
                    }

                    this.Basic_Chart.Series[0].ItemsSource = source;
                    this.Basic_Chart.Series[1].ItemsSource = standardSource;
                    GenerateTableAgain_Click(new object(), new RoutedEventArgs());

                    Basic_Chart.Visibility = Visibility.Visible;
                    AnalysisGrid.Visibility = Visibility.Visible;
                    InfoListView.Visibility = Visibility.Visible;
                    Debug.WriteLine(y_b1.Count);
                }
            }
            catch (Exception ex)
            {
        ...
        private void BottomToggle_Toggled(object sender, RoutedEventArgs e)
        {
            ...
            if (toggleSwitch != null)
            {
                ...
                Debug.WriteLine(showBottomFlag);
            }
        }
